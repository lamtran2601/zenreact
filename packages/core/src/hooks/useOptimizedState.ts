...
import { OptimizedStateHookResult } from '../types';
 ...
 * ```
 */
export function useOptimizedState<T>(initialState: T): OptimizedStateHookResult<T> {
  const [state, setState] = useState<T>(initialState);

  function setOptimizedState(value: T) {
